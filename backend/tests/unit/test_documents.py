...
class TestIndexDocument:
    def test_mismatched_embeddings_raises(self):
        ...
        chunks = [
            Chunk(content="one", section=None, chunk_type="paragraph", page_number=1),
            Chunk(content="two", section=None, chunk_type="paragraph", page_number=1),
        ]

        fake_conn = type("C", (), {"cursor": lambda self: _FakeCursor(), "commit": lambda self: None})()

        with pytest.raises(ValueError):
            index_document(fake_conn, "doc", "policy", "general", None, chunks, embeddings=[[0.1] * 384])

    def test_upsert_uses_content_hash_conflict_target(self):
        from app.documents.indexing import index_document
        from app.documents.chunking.structural_chunker import Chunk

        chunks = [
            Chunk(content="chunk one content", section=None, chunk_type="paragraph", page_number=1),
        ]
        conn = type("C", (), {"cursor": lambda self: _FakeCursor(), "commit": lambda self: None})()
        idx = index_document(conn, "doc", "policy", "general", None, chunks)
        assert "ON CONFLICT (content_hash)" in _FakeCursor.captured["sql"]
        assert _FakeCursor.captured["params"][0] == 1  # document_id from RETURNING
...
class _FakeCursor:
    ...
    def execute(self, sql, params=None):
        if "INSERT INTO document_chunks" in sql:
            _FakeCursor.captured["sql"] = sql
            _FakeCursor.captured["params"] = params
            self.rowcount = 1
        return self
