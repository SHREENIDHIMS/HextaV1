...
async def search(
    ...
    # Phase 4b: Optional cross-encoder rerank of the top-K RRF candidates.
    # Gated by settings.rerank_enabled — a no-op passthrough when disabled.
    from app.ranking.reranker import rerank_candidates

    ranked = rerank_candidates(ranked, request.query)
    ...
    return SearchResponse(
        ...
        related_questions=_related_from_sources(package),
    )


def _related_from_sources(package: "ResponsePackage") -> list[str]:
    """Deterministic, extractive follow-ups: unique section headings from
    the retrieved evidence, prefixed by their document title. Never
    generates or paraphrases question text — only reuses verbatim
    headings from the retrieved chunks."""
    seen: list[str] = []
    for e in package.excerpts:
        if not e.source.section:
            continue
        label = e.source.section.strip()
